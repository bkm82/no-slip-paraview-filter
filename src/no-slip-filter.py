...
class NoSlipFilter(VTKPythonAlgorithmBase):
    ...
    def RequestData(self, request, inInfo, outInfo):
        ...
        # Find all of the points in the volume mesh that are on the surface
        # mesh by first finding all of the points in the Volume_Input that are
        # also in Wall_Input. This will create a mask vector that is the same
        # number of rows as the Volume input. If that point is on the surface
        ...
        velocity[np.where(mask), :] = 0  # velocity to zero for surface rows
